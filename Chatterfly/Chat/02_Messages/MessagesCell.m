...
//	imageUser.layer.masksToBounds = YES;
    ...
    if([lastUser.objectId isEqualToString:[PFUser currentUser].objectId])
        lastUser = message[PF_MESSAGES_USER];
    [lastUser fetch];

    [imageUser setImageWithURL:[lastUser[PF_USER_PICTURE] url] placeholderImage:[UIImage imageNamed:@"placeholder_gb.png"]];
    
	//---------------------------------------------------------------------------------------------------------------------------------------------
	labelDescription.text = lastUser[PF_USER_USERNAME];
    ...
    if(counter == 0) {
        ...
        labelCounter.layer.cornerRadius = labelCounter.frame.size.height/2;
        labelCounter.layer.masksToBounds = YES;
        labelCounter.backgroundColor = [UIColor redColor];
    }
}
